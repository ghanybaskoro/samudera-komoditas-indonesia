...
export default function Home() {
  ...
  const content = {
    id: {
      ...
      legalItems: [
        ...
        { label: "Alamat Terdaftar", value: footerInfo.address }, // Ambil dari state
      ],
    ...
    en: {
      ...
      legalItems: [
        ...
        { label: "Registered Address", value: footerInfo.address }, // Ambil dari state
      ],
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-800 flex flex-col">
      {/* HERO & NAVBAR TETAP SAMA SEPERTI SEBELUMNYA */}
      ...
      {/* SECTIONS VISI MISI, ABOUT, LEGALITAS (TETAP SAMA) */}
      ...
      <section className="py-20 px-6 bg-slate-50 border-t border-slate-200">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-blue-900 mb-4">{t.legalTitle}</h2>
            <p className="text-gray-600 max-w-2xl mx-auto">{t.legalDesc}</p>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            {t.legalItems.map((item, index) => (
              <div key={index} className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col hover:border-blue-300 transition-colors">
                <span className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">{item.label}</span>
                <span className="text-lg font-medium text-slate-800 break-words">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-4">
              <div className="relative w-10 h-10">
                <Image src="/logo.png" alt="Logo SKI" fill className="object-contain" />
              </div>
              <span className="font-bold text-white text-lg leading-tight">
                Samudera Komoditas <br /> Indonesia
              </span>
            </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm">
              ...
              <li className="flex items-center gap-3">
                <span className="text-xl">📧</span>
                <a href={`mailto:${footerInfo.email}`} className="hover:text-white transition">
                  {footerInfo.email}
                </a>
              </li>
              <li className="flex items-center gap-3">
                <span className="text-xl">📞</span>
                <a href={`tel:${footerInfo.phone}`} className="hover:text-white transition">
                  {footerInfo.phone}
                </a>
              </li>
            </ul>
